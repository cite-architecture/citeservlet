...
var IIP = new Class({
   ...
   */
    ...
    this.imgURN = options['imgUrn'];

    
    // ************************
    // CWB - Split out ROI
    if ((options['citeROI']) && ( options['citeROI'].split(",").length == 4 )){ 
	    if( options['citeROI'].split(",").length == 4 ){
		    this.citeROI = new Object();
		    this.citeROI.left = options['citeROI'].split(",")[0];
		    this.citeROI.top = options['citeROI'].split(",")[1];
		    this.citeROI.width = options['citeROI'].split(",")[2];
		    this.citeROI.height = options['citeROI'].split(",")[3];

				// test ROI
				if  	(((this.citeROI.left < 0) || (this.citeROI.left > 1)) ||
					((this.citeROI.top < 0) || (this.citeROI.left > 1)) ||
					((this.citeROI.width < 0) || (this.citeROI.left > 1)) ||
					((this.citeROI.height < 0) || (this.citeROI.left > 1))) {
						alert("The requested region-of-interest contained invalid values.");    	
						this.citeROI = null;    	
				}

		} else {
			alert("The requested region-of-interest was not formed correctly: " + options['citeROI']);
			this.citeROI = null;
		}
  ...
  createWindows: function(){
    ...
    this.highlightROI(); // CWB - Initialize ROI rectangle, if requested
    ...
    new Element('img', {src: 'css/imgs/iip.32x32.png', id: 'info', styles: { opacity: 0.8 } } ).injectInside('logo');

    // Fix IE7 PNG transparency problem
    if( Browser.Engine.trident5 ){
      $('info').setStyle( 'filter', 'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src="css/imgs/iip.32x32.png",sizingMethod=scale)' );
    }
  ...
  createNavigationWindow: function() {
    ...
    // **** CWB - create itty-bitty ROI inside navwin
    if (this.citeROI){
	    var ittyBittyROI = new Element( 'div', {
		id: 'ittyBittyROI',
		styles: {
			width: this.min_x * this.citeROI.width,
			height: this.min_y * this.citeROI.height,
			opacity: 0.2,
			top: (this.min_y * this.citeROI.top) + 10, //make space for drag-bar at top
			left: this.min_x * this.citeROI.left
		}
	    });
	    ittyBittyROI.injectInside( navwin );
	}
    //
   ...
   var loadBarContainer = new Element('div', {
	...
	id: 'navbuttons',
	html: '<img id="shiftLeft" src="css/imgs/left.png"/><img id="shiftUp" src="css/imgs/up.png"/><img id="shiftRight" src="css/imgs/right.png"/><br/><img id="shiftDown" src="css/imgs/down.png"/><br/><img id="zoomIn" src="css/imgs/zoomIn.png"/><img id="zoomOut" src="css/imgs/zoomOut.png"/><img id="reset" src="css/imgs/reset.png"/>'
    });
  ...
  positionZone: function(){
	...
	// **********************************************
	// CWB - ROI functions below!
	
	
	
	highlightROI: function(){
		
			if (this.citeROI != null){
			var ROIZone = new Element( 'div', {
			
				id: 'ROIzone',
				styles: {
					width: this.citeROI.width,
					height: this.citeROI.height,
					opacity: 0.2,
					top: this.citeROI.top,
					left: this.citeROI.left,
				},
				morph: {
					duration: 200,
					transition: Fx.Transitions.Quad.easeInOut
			...
			ROIZone.addEvent( 'dblclick',this.zoom.bind(this) );
			
			ROIZone.addEvent( 'click',function(tempURL){ 
			
				var tempURL = "@images@?request=GetBinaryImage&urn=" + iip.imgURN;
				tempURL += "&w=8000";				
				open(tempURL);
			
			} );
			
			ROIZone.injectAfter(this.source);

		}
    		
	},
	
	positionROI: function(){
		
		if (this.citeROI != null){
			var myBasicLeft = this.max_width / Math.pow(2,(this.num_resolutions - this.res - 1)) * this.citeROI.left;
			var myBasicTop = this.max_height / Math.pow(2,(this.num_resolutions - this.res - 1)) * this.citeROI.top;
			var myBasicWidth = this.max_width / Math.pow(2,(this.num_resolutions - this.res - 1)) * this.citeROI.width;
			var myBasicHeight = this.max_height / Math.pow(2,(this.num_resolutions - this.res - 1)) * this.citeROI.height;
			...
			pleft = myBasicLeft - this.rgn_x - xoffset;
			ptop = myBasicTop - this.rgn_y - yoffset;
			pwidth = myBasicWidth;
			pheight = myBasicHeight;
			
			
			// Move the zone to the new size and position
			$('ROIzone').morph({
				left: pleft,
				top: ptop, // 10px for the toolbar
				width: pwidth,
				height: pheight
			});
